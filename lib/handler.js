import { markTransactionSent, insertWebhookEvent } from './db.js';
...
export async function handleWebhook(payload, headers) {
  ...
  const inserted = await insertWebhookEvent(eventType, payload, headers)

  if (inserted) {
    console.log(`✅ Webhook event stored (id: ${inserted.id})`)
  } else {
    console.warn('⚠️ Failed to insert webhook event')
  }
