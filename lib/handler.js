...
import { insertWebhookEvent, isTransactionProcessed, createProcessedTransaction, markTransactionProcessed, markTransactionFailed } from './db.js';
...
async function processReservationTransaction(transaction, propertyId) {
  ...
  console.log(`Processing reservation transaction: ${reservationId} (tx: ${txId})`);
  ...
  try {
    ...
    if (!grandTotal || !paid) {
      await markTransactionFailed(txId, new Error('Missing balance information'));
      return { 
        status: 'skipped',
...
export async function handleWebhook(payload) {
  try {
    ...
    const webhookEvent = await insertWebhookEvent(payload.eventType || 'accounting/transaction', payload);
    
    // Create processed transaction record with pending status
    const processedTransaction = await createProcessedTransaction(webhookEvent?.id, txId);
